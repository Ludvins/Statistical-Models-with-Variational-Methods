...
\begin{definition}
A \emph{belief network or Bayesian network} is a pair \(\mathcal{B} = (G, P)\)
...
\begin{definition} Let \(G\) be a DAG, and \(x \in V\) be a vertice such
that \(\forall y \in ne(X), (y,x)\in E\), then \(x\) is called
a \emph{collider}.

For example in figure \ref{fig:relations}, \(x_2\) is a collider.
\end{definition}
