Consider we have \(N\) variables \( X_1,dots,X_N \) with the corresponding distribution \(P(x_1,\dots,x_N)\). Let \(\mathcal{E}\) be a set of indexes such as \texttt{evidence}
...
underlying idea of bayesian networks is to specify which variables are independent
of others, factorizing the joint probability distribution.
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[
    ...
    \end{tikzpicture}
    \captionof{figure}{Bayesian Network factorizing \(P(x_1, x_2, x_3, x_4) = P(x_1 \mid x_4)P(x_2\mid x_3, x_4)P(x_3)P(x_4)\)}
    \label{fig:bn_example}
\end{figure}

Given this definition, the representation of any Bayesian network is a directed acyclic graph.

Any probability distribution can be written as a Bayesian Network, even though
it may end up been a fully-connected "cascase" DAG, which means that each variable \( X_n \) is a parent of any \( X_m \) with \( m > n \). Using four variables:
\[
...
tables \(P(x_i\mid pa(x_i))\). When the number of variables is large, this is still
...
represented as \(x \to y\) in a DAG (a Bayesian network can be defined giving the graph or the joint probability).
It may appear to encode dependence between both variables but the
conditional \(P(y\mid x)\) could happen to equal \(P(y)\), giving \(P(x,y) = P(x)P(y)\).
...
on \(\bm{Z}\) in all probability distributions that factorize over \( G \) .
\end{theorem}
...
In some situations our Bayesian networks will contain a number of nodes that are
essentially the same but repeated a number of times, in this cases we are using the \emph{plate notation}. Suppose we have the situation that figure
