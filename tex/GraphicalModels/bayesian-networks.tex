...
\begin{algorithm}[t]
  \SetAlgoLined\KwData{Set of factors \(\bm{\phi}\) in which the joint distribution factorizes, set of variables to be eliminates \(\bX\) and an ordering on \(\bX=(X_{1},\dots,X_{N})\) such that the set is ordered \(X_{i} < X_{j} \iff i < j\).}
  \KwResult{A factor \(\psi^{*}\) with the variables eliminated.}
  \For{\(n = 1,\dots,N\) \tcp*{Eliminate variable \(X_{n}\) }}{
    Let \(\bm{\phi} '\) be the set of factors that do depend on \(X_{n}\)\;
